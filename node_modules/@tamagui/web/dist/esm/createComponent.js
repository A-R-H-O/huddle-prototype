...
function createComponent(staticConfig) {
  ...
  const component = React.forwardRef((propsIn, forwardedRef) => {
    ...
    const animationDriver = componentContext.animationDriver, useAnimations = animationDriver?.useAnimations, componentState = useComponentState(
      props,
      componentContext,
      staticConfig,
      config
    ...
    if (hasAnimationProp && animationDriver?.avoidReRenders) {
      ...
      setStateShallow = (next) => {
        ...
        if (Object.keys(next).every(
          ...
          const updatedState = { ...state, ...next }, nextStyles = getSplitStyles(
            props,
            staticConfig,
            theme,
            themeName,
...
` && console.error(
    ...
    process.env.NODE_ENV === "development" && isWeb && useIsomorphicLayoutEffect(() => {
      ...
      let tm;
      if (state.unmounted) {
        if ((animationDriver?.supportsCSSVars ?? config?.animations?.supportsCSSVars) || isAndroid)
