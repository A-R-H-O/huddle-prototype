  ...
  var _loop = function (keyOg2) {
    ...
    if (isMediaOrPseudo && keyInit.startsWith("$group-")) {
      ...
      parts.length === 2 || parts.length === 3 && pseudoPriorities[parts[parts.length - 1]]) {
        var name2 = parts[1];
        componentContext?.groups.subscribe && !componentContext?.groups.state[name2] && (keyInit = keyInit.replace("$group-", "$group-true-"));
      }
    }
    ...
    if (propMapper(keyInit, valInit, styleState, disablePropMap, function (key4, val2) {
      ...
      if (process.env.NODE_ENV === "development" && debug === "verbose" && (console.groupCollapsed("  \u{1F4A0} expanded", keyInit, "=>", key4), log(val2), console.groupEnd()), val2 != null && !(key4 in usedKeys)) {
            ...
            try {
              for (var _iterator12 = mediaStyles[Symbol.iterator](), _step12; !(_iteratorNormalCompletion12 = (_step12 = _iterator12.next()).done); _iteratorNormalCompletion12 = !0) {
                ...
                if (!(isSubStyle && !isActivePlatform(property))) {
                  ...
                  fullKey1 in usedKeys || (addStyleToInsertRules(rulesToInsert, out, startedUnhydrated), classNames[fullKey1] = out[StyleObjectIdentifier]);
                }
              }
            } catch (err) {
            ...
            if (isThemeMedia) {
              ...
              if (groupPseudoKey) {
                ...
                var componentGroupPseudoState = (componentGroupState ||
                  ...
                  componentContext.groups.state[groupName]).pseudo,
