...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, elementType, startedUnhydrated, debug) => {
  ...
  const {
    ...
    usedKeys = {},
    shouldDoClasses = acceptsClassName && isWeb && !styleProps.noClass && !styleProps.isAnimated,
    rulesToInsert = {},
    classNames = {};
  let pseudos = null,
    space = props.space,
  ...
  process.env.NODE_ENV === "development" && debug === "profile" && time`style-state`, process.env.NODE_ENV === "development" && debug && debug !== "profile" && isClient && (console.groupCollapsed("getSplitStyles (collapsed)"), log({
    props,
    staticConfig,
    shouldDoClasses,
    styleProps,
  ...
  for (const keyOg in props) {
    ...
    if (process.env.NODE_ENV, disableExpandShorthands || keyInit in shorthands && (keyInit = shorthands[keyInit]), keyInit === "className" || keyInit in usedKeys || asChild && webViewFlexCompatStyles[keyInit] === valInit) continue;
    ...
    if (propMapper(keyInit, valInit, styleState, disablePropMap, (key, val) => {
      ...
      if (process.env.NODE_ENV === "development" && debug === "verbose" && (console.groupCollapsed("  \u{1F4A0} expanded", keyInit, "=>", key), log(val), console.groupEnd()), val == null || key in usedKeys) return;
      ...
      if (isPseudo) {
        ...
        if (shouldDoClasses && !isExit) {
          ...
          for (const psuedoStyle of pseudoStyles) {
            ...
            fullKey in usedKeys || (addStyleToInsertRules(rulesToInsert, psuedoStyle, startedUnhydrated), classNames[fullKey] = psuedoStyle[StyleObjectIdentifier]);
          }
        }
        if (!shouldDoClasses || isExit || isEnter) {
          ...
          for (const pkey in pseudoStyleObject) {
            ...
            if (isDisabled) applyDefaultStyle(pkey, styleState);else {
              ...
              shouldMerge && (process.env.IS_STATIC === "is_static" && (pseudos ||= {}, pseudos[key] ||= {}, pseudos[key][pkey] = val2), mergeStyle(styleState, pkey, val2)), process.env.NODE_ENV === "development" && debug === "verbose" && log("    subKey", pkey, shouldMerge, {
                importance,
                curImportance,
                pkey,
                val: val2
      ...
      if (isMedia) {
        ...
        if (process.env.NODE_ENV === "development" && debug === "verbose" && log(`  \u{1F4FA} ${key}`, {
          ...
          for (const style of mediaStyles) {
            ...
            fullKey in usedKeys || (addStyleToInsertRules(rulesToInsert, out, startedUnhydrated), classNames[fullKey] = out[StyleObjectIdentifier]);
          }
        } else {
          ...
          if (isThemeMedia) {
            ...
            if (groupPseudoKey) {
              ...
              const isActive = (componentGroupState ||
                ...
                priority = pseudoPriorities[groupPseudoKey];
              if (process.env.NODE_ENV === "development" && debug === "verbose" && log(` \u{1F3D8}\uFE0F GROUP pseudo ${groupMediaKey} active? ${isActive}, priority ${priority}`), !isActive) {
                for (const pkey in mediaStyle) applyDefaultStyle(pkey, styleState);
                return;
              }
  ...
  if (!styleProps.noNormalize && !staticConfig.isReactNative && !staticConfig.isHOC && (!styleProps.isAnimated || animationDriver.supportsCSSVars) && Array.isArray(styleState.style?.transform) && (styleState.style.transform = transformsToString(styleState.style.transform)), !styleProps.noMergeStyle && styleState.style && shouldDoClasses) {
    let retainedStyles,
      shouldRetain = !1;
    if (!styleState.style.$$css) {
      ...
      for (const atomicStyle of atomic) {
        ...
        isAnimatedAndAnimateOnly ? (retainedStyles ||= {}, retainedStyles[key] = styleState.style[key]) : nonAnimatedAnimateOnly ? (retainedStyles ||= {}, retainedStyles[key] = value, shouldRetain = !0) : (addStyleToInsertRules(rulesToInsert, atomicStyle, startedUnhydrated), classNames[key] = identifier);
      }
  ...
  if (!styleProps.noMergeStyle && !asChildExceptStyleLike) {
    const style = styleState.style;
    {
      ...
      if (styleProps.isAnimated) style && (viewProps.style = style);else if (isReactNative) {
        const cnStyles = {
          $$css: !0
        };
...
function mergeStyle(styleState, key, val, disableNormalize = !1) {
  ...
  if (key in stylePropsTransform) styleState.flatTransforms ||= {}, styleState.flatTransforms[key] = val;else {
...
const getSubStyle = (styleState, subKey, styleIn, avoidMergeTransform) => {
  ...
  useSplitStyles = (a, b, c, d, e, f, g, h, i, j, k) => {
    ...
    return j || useInsertEffectCompat(() => {
      insertStyleRules(res.rulesToInsert);
    }, [res.rulesToInsert]), res;
  };
function addStyleToInsertRules(rulesToInsert, styleObject, startedUnhydrated = !1) {
  {
    const identifier = styleObject[StyleObjectIdentifier];
    startedUnhydrated ? rulesToInsert[identifier] = styleObject : shouldInsertStyleRules(identifier) && (updateRules(identifier, styleObject[StyleObjectRules]), rulesToInsert[identifier] = styleObject);
  }
}
