...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, elementType, startedUnhydrated, debug) => {
  ...
  const { shorthands } = conf, {
    ...
    parentStaticConfig,
    acceptsClassName
  } = staticConfig, viewProps = {}, mediaState = styleProps.mediaState || globalMediaState, usedKeys = {}, shouldDoClasses = acceptsClassName && isWeb && !styleProps.noClass && !styleProps.isAnimated, rulesToInsert = {}, classNames = {};
  ...
  for (const keyOg in props) {
    ...
    if (process.env.NODE_ENV, disableExpandShorthands || keyInit in shorthands && (keyInit = shorthands[keyInit]), keyInit === "className" || keyInit in usedKeys || asChild && webViewFlexCompatStyles[keyInit] === valInit)
      continue;
      ...
      if (isMedia) {
        ...
        if (process.env.NODE_ENV === "development" && debug === "verbose" && log(`  \u{1F4FA} ${key}`, {
          ...
          if (isThemeMedia) {
            ...
            const groupInfo = getGroupPropParts(mediaKeyShort), groupName = groupInfo.name, groupContext = componentContext?.groups.state[groupName];
            if (!groupContext) {
            ...
            if (groupPseudoKey) {
              ...
              componentContext.groups.state[groupName]).pseudo?.[groupPseudoKey], priority = pseudoPriorities[groupPseudoKey];
