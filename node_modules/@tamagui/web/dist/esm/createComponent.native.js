...
function createComponent(staticConfig) {
  ...
  var component = /* @__PURE__ */React.forwardRef(function (propsIn, forwardedRef) {
    ...
    if (context && (contextValue = React.useContext(context), contextValue)) {
      process.env.NODE_ENV === "development" && propsIn?.debug === "verbose" && log(" \u{1F447} contextValue", contextValue);
    ...
    var animationDriver = componentContext.animationDriver,
      ...
      componentState = useComponentState(props, componentContext, staticConfig, config),
      {
        curStateRef,
        disabled,
        groupName,
    ...
    if ("themeInverse" in props && (themeStateProps.inverse = props.themeInverse), "theme" in props && (themeStateProps.name = props.theme), typeof curStateRef.isListeningToTheme == "boolean" && (themeStateProps.needsUpdate = function () {
      ...
      if (console.info(`%c ${banner} (hydrated: ${isHydrated}) (unmounted: ${state.unmounted})`, "background: green; color: white;"), isServer) log({
        ...
        childLog.length && (childLog = `(children: ${childLog})`), console.groupCollapsed(`${childLog} [inspect props, state, context \u{1F447}]`), log("props in:", propsIn), log("final props:", props), log({
          state,
          staticConfig,
          elementType,
          themeStateProps
    ...
    var splitStyles = useSplitStyles(props, staticConfig, theme, themeName, state, styleProps, null, componentContext, elementType, startedUnhydrated, debugProp);
    if (hasAnimationProp && animationDriver?.avoidReRenders) {
      var styleListener = stateRef.current.useStyleListener,
        ogSetStateShallow = setStateShallow;
      setStateShallow = function (next) {
        var avoidReRenderKeys = /* @__PURE__ */new Set(["hover", "press", "pressIn"]),
        ...
        if (canAvoidReRender && styleListener) {
          var updatedState = {
            ...
            nextStyles = getSplitStyles(props, staticConfig, theme, themeName, updatedState, styleProps, null, componentContext, elementType, startedUnhydrated, debugProp);
    ...
    if (shouldUseAnimation) {
      ...
      animations && (animationStyles = animations.style, viewProps.style = animationStyles, animations.className && (viewProps.className = `${state.unmounted === "should-enter" ? "t_unmounted " : ""}${viewProps.className || ""} ${animations.className}`), animations.ref && (animatedRef = animations.ref)), process.env.NODE_ENV === "development" && time && time`animations`;
    }
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`), process.env.NODE_ENV === "development" && time && time`destructure`, groupName && (nonTamaguiProps.onLayout = composeEventHandlers(nonTamaguiProps.onLayout, function (e) {
    ...
    }, forwardedRef, setElementProps, animatedRef)), viewProps.ref = curStateRef.composedRef, process.env.NODE_ENV === "development" && !isReactNative && !isText && isWeb && !isHOC && React.Children.toArray(props.children).forEach(function (item) {
...
` && console.error(`Unexpected text node: ${item}. A text node cannot be a child of a <${staticConfig.componentName || propsIn.tag || "View"}>.`, props);
    ...
    process.env.NODE_ENV === "development" && isWeb && useIsomorphicLayoutEffect(function () {
      ...
      if (state.unmounted) {
        ...
        if (((_animationDriver_supportsCSSVars = animationDriver?.supportsCSSVars) !== null && _animationDriver_supportsCSSVars !== void 0 ? _animationDriver_supportsCSSVars : !(config == null || (_config_animations2 = config.animations) === null || _config_animations2 === void 0) && _config_animations2.supportsCSSVars) || isAndroid) return tm = setTimeout(function () {
          setStateShallow({
            unmounted: !1
          });
        }), function () {
      ...
      var dispose = !disabled && (pseudoGroups || mediaGroups) ? subscribeToContextGroup({
        componentContext,
        setStateShallow,
        state,
        mediaGroups,
