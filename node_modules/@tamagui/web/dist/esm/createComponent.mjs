...
function createComponent(staticConfig) {
  ...
  const component = React.forwardRef((propsIn, forwardedRef) => {
    ...
    const splitStyles = useSplitStyles(props, staticConfig, theme, themeName, state, styleProps, null, componentContext, elementType, startedUnhydrated, debugProp);
...
` && console.error(`Unexpected text node: ${item}. A text node cannot be a child of a <${staticConfig.componentName || propsIn.tag || "View"}>.`, props);
    ...
    process.env.NODE_ENV === "development" && isWeb && useIsomorphicLayoutEffect(() => {
      ...
      let tm;
      if (state.unmounted) {
        if ((animationDriver?.supportsCSSVars ?? config?.animations?.supportsCSSVars) || isAndroid) return tm = setTimeout(() => {
          setStateShallow({
            unmounted: !1
          });
        }), () => clearTimeout(tm);
      ...
      const dispose = !disabled && (pseudoGroups || mediaGroups) ? subscribeToContextGroup({
        componentContext,
        setStateShallow,
        state,
        mediaGroups,
    ...
    if (process.env.NODE_ENV === "development" && time && time`context-override`, startedUnhydrated) {
      const styleTags = useMemo(() => getStyleTags(Object.values(splitStyles.rulesToInsert)), []);
