  ...
  addEventListener("mouseup", cancelTouches), addEventListener("touchend", cancelTouches), addEventListener("touchcancel", cancelTouches), process.env.NODE_ENV === "development" && (startVisualizer = () => {
    ...
    if (devVisualizerConfig) {
      ...
      window.addEventListener("blur", () => {
        ...
        defaultPrevented || key === options.key && isShowing && show(!1);
      });
    }
  });
}
...
function createComponent(staticConfig) {
  ...
  const component = import_react.default.forwardRef((propsIn, forwardedRef) => {
    ...
    if (context && (contextValue = import_react.default.useContext(context), contextValue)) {
      process.env.NODE_ENV === "development" && propsIn?.debug === "verbose" && (0, import_log.log)(" \u{1F447} contextValue", contextValue);
    ...
    const animationDriver = componentContext.animationDriver, useAnimations = animationDriver?.useAnimations, componentState = (0, import_useComponentState.useComponentState)(
      props,
      componentContext,
      staticConfig,
      config
    ...
    if ("themeInverse" in props && (themeStateProps.inverse = props.themeInverse), "theme" in props && (themeStateProps.name = props.theme), typeof curStateRef.isListeningToTheme == "boolean" && (themeStateProps.needsUpdate = () => !!stateRef.current.isListeningToTheme), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      ...
      if (console.info(
        ...
        childLog.length && (childLog = `(children: ${childLog})`), console.groupCollapsed(`${childLog} [inspect props, state, context \u{1F447}]`), (0, import_log.log)("props in:", propsIn), (0, import_log.log)("final props:", props), (0, import_log.log)({ state, staticConfig, elementType, themeStateProps }), (0, import_log.log)({ contextProps: styledContextProps, overriddenContextProps }), (0, import_log.log)({ presence, isAnimated, isHOC, hasAnimationProp, useAnimations }), console.groupEnd();
      }
    }
    ...
    if (hasAnimationProp && animationDriver?.avoidReRenders) {
      const styleListener = stateRef.current.useStyleListener, ogSetStateShallow = setStateShallow;
      setStateShallow = (next) => {
        const avoidReRenderKeys = /* @__PURE__ */ new Set(["hover", "press", "pressIn"]);
        if (Object.keys(next).every(
          (key) => avoidReRenderKeys.has(key)
        ) && styleListener) {
          const updatedState = { ...state, ...next }, nextStyles = (0, import_getSplitStyles.getSplitStyles)(
            props,
            staticConfig,
            theme,
            themeName,
...
` && console.error(
    ...
    const { pseudoGroups, mediaGroups } = splitStyles, unPress = () => {
    ...
    process.env.NODE_ENV === "development" && import_constants.isWeb && (0, import_constants.useIsomorphicLayoutEffect)(() => {
      ...
      let tm;
      if (state.unmounted) {
        if ((animationDriver?.supportsCSSVars ?? config?.animations?.supportsCSSVars) || import_constants.isAndroid)
    ...
    if (process.env.NODE_ENV === "development" && time && time`context-override`, startedUnhydrated) {
      const styleTags = (0, import_react.useMemo)(() => (0, import_wrapStyleTags.getStyleTags)(Object.values(splitStyles.rulesToInsert)), []);
