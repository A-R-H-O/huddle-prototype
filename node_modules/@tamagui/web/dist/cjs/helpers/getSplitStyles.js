...
const getSplitStyles = (props, staticConfig, theme, themeName, componentState, styleProps, parentSplitStyles, componentContext, elementType, startedUnhydrated, debug) => {
  ...
  const { shorthands } = conf, {
    ...
    parentStaticConfig,
    acceptsClassName
  } = staticConfig, viewProps = {}, mediaState = styleProps.mediaState || import_useMedia.mediaState, usedKeys = {}, shouldDoClasses = acceptsClassName && import_constants.isWeb && !styleProps.noClass && !styleProps.isAnimated, rulesToInsert = {}, classNames = {};
  ...
  for (const keyOg in props) {
    ...
    if (process.env.NODE_ENV, disableExpandShorthands || keyInit in shorthands && (keyInit = shorthands[keyInit]), keyInit === "className" || keyInit in usedKeys || asChild && import_constants2.webViewFlexCompatStyles[keyInit] === valInit)
      continue;
    ...
    if ((0, import_propMapper.propMapper)(keyInit, valInit, styleState, disablePropMap, (key, val) => {
      ...
      if (isMedia) {
        ...
        if (process.env.NODE_ENV === "development" && debug === "verbose" && (0, import_log.log)(`  \u{1F4FA} ${key}`, {
          ...
          const mediaStyles = (0, import_getCSSStylesAtomic.getCSSStylesAtomic)(mediaStyle), priority = mediaStylesSeen;
          mediaStylesSeen += 1;
          for (const style of mediaStyles) {
            ...
            fullKey in usedKeys || (addStyleToInsertRules(rulesToInsert, out, startedUnhydrated), classNames[fullKey] = out[import_helpers.StyleObjectIdentifier]);
          }
        } else {
          ...
          if (isThemeMedia) {
            ...
            const groupInfo = (0, import_getGroupPropParts.getGroupPropParts)(mediaKeyShort), groupName = groupInfo.name, groupContext = componentContext?.groups.state[groupName];
            if (!groupContext) {
            ...
            if (groupPseudoKey) {
              ...
              componentContext.groups.state[groupName]).pseudo?.[groupPseudoKey], priority = import_pseudoDescriptors.pseudoPriorities[groupPseudoKey];
  ...
  if (!styleProps.noNormalize && !staticConfig.isReactNative && !staticConfig.isHOC && (!styleProps.isAnimated || animationDriver.supportsCSSVars) && Array.isArray(styleState.style?.transform) && (styleState.style.transform = (0, import_transformsToString.transformsToString)(styleState.style.transform)), !styleProps.noMergeStyle && styleState.style && shouldDoClasses) {
...
function addStyleToInsertRules(rulesToInsert, styleObject, startedUnhydrated = !1) {
  {
    ...
    startedUnhydrated ? rulesToInsert[identifier] = styleObject : (0, import_insertStyleRule.shouldInsertStyleRules)(identifier) && ((0, import_insertStyleRule.updateRules)(identifier, styleObject[import_helpers.StyleObjectRules]), rulesToInsert[identifier] = styleObject);
  }
}
