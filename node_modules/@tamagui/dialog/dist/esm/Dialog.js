...
const DIALOG_NAME = "Dialog", [createDialogContext, createDialogScope] = createContextScope(DIALOG_NAME), [DialogProvider, useDialogContext] = createDialogContext(DIALOG_NAME), TRIGGER_NAME = "DialogTrigger", DialogTriggerFrame = styled(View, {
  ...
  const { __scopeDialog, children, space, spaceDirection, separator } = props, themeName = useThemeName(), context = useDialogContext(PORTAL_NAME, props.__scopeDialog), isAdapted = useAdaptIsActive();
  ...
  const content = /* @__PURE__ */ jsx(DialogProvider, { scope: __scopeDialog, ...context, children: /* @__PURE__ */ jsx(Theme, { name: themeName, children: childrenSpaced }) });
  return isAdapted ? /* @__PURE__ */ jsx(AdaptPortalContents, { children: content }) : context.modal ? /* @__PURE__ */ jsx(PortalItem, { hostName: context.modal ? "root" : context.adaptName, children: content }) : content;
}, DialogPortal = (props) => {
  const { __scopeDialog, forceMount, children, ...frameProps } = props, context = useDialogContext(PORTAL_NAME, __scopeDialog), isShowing = forceMount || context.open, [isFullyHidden, setIsFullyHidden] = React.useState(!isShowing), isAdapted = useAdaptIsActive();
  isShowing && isFullyHidden && setIsFullyHidden(!1);
  const handleExitComplete = React.useCallback(() => {
    setIsFullyHidden(!0);
  }, []), zIndex = getExpandedShorthand("zIndex", props), contents = /* @__PURE__ */ jsx(StackZIndexContext, { zIndex: resolveViewZIndex(zIndex), children: /* @__PURE__ */ jsx(AnimatePresence, { onExitComplete: handleExitComplete, children: isShowing || isAdapted ? children : null }) });
  ...
  return isWeb ? /* @__PURE__ */ jsx(
    Portal,
    {
      zIndex,
      stackZIndex: 1e3,
      children: /* @__PURE__ */ jsx(PassthroughTheme, { children: framedContents })
    }
  ) : isAdapted ? framedContents : /* @__PURE__ */ jsx(DialogPortalItem, { __scopeDialog, children: framedContents });
  ...
  React.forwardRef(function({ __scopeDialog, ...props }, forwardedRef) {
    const portalContext = usePortalContext(OVERLAY_NAME, __scopeDialog), { forceMount = portalContext.forceMount, ...overlayProps } = props, context = useDialogContext(OVERLAY_NAME, __scopeDialog), isAdapted = useAdaptIsActive();
  ...
  (props, forwardedRef) => {
    ...
    if (useAdaptIsActive())
      return !isWeb && !context.open ? null : /* @__PURE__ */ jsx(DialogPortalItem, { children: contentProps.children });
...
If you want to hide the \`${titleWarningContext.titleName}\`, you can wrap it with our VisuallyHidden component.`;
  ...
  React.forwardRef(function(props, ref) {
    ...
    } = props, baseId = React.useId(), scopeId = `scope-${baseId}`, contentId = `content-${baseId}`, titleId = `title-${baseId}`, descriptionId = `description-${baseId}`, scopeKey = __scopeDialog ? Object.keys(__scopeDialog)[0] : scopeId, adaptName = getAdaptName({ scopeKey, contentId }), triggerRef = React.useRef(null), contentRef = React.useRef(null), [open, setOpen] = useControllableState({
      prop: openProp,
    ...
    return React.useImperativeHandle(
      ...
      AdaptParent,
      {
        ...
        children: /* @__PURE__ */ jsx(DialogProvider, { ...context, children: /* @__PURE__ */ jsx(DialogSheetController, { onOpenChange: setOpen, __scopeDialog, children }) })
      }
    );
  }),
  {
  ...
  const context = useDialogContext("DialogSheetController", props.__scopeDialog), isAdapted = useAdaptIsActive();
  return /* @__PURE__ */ jsx(
    SheetController,
    {
      onOpenChange: (val) => {
